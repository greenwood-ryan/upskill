...
public class BinaryTree{
	...
	public static void main(String[] args){
		...
		tb = new BinaryTree();
		tb.testInsertNodesLevelOrderTree();
	...
	void testInsertNodesLevelOrderTree(){
		int[] keys = {5,8,9,4,6,3,1,7,2,5};
		insertNodesLevelOrderBST(keys);
		printInOrder(root);
	}
	...
	public void insertNodesLevelOrderBST(int[] keys){
		insertLevelOrderTree(root, keys[0]);
		for(int i=1; i<keys.length; i++)
			insertLevelOrderTree(root, keys[i]);
	}
	
	public void insertLevelOrderTree(Node root, int key){
		if(root == null){
